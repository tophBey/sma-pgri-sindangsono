...
<div class="lg:w-[77.5%] lg:h-full md:w-full sm:w-full overflow-hidden">
    ...
    <div class="  min-h-[900px] lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px]  overflow-hidden">
        <p class="font-signika font-medium text-[24px]">Tambah Ekstrakulikuler</p>
    ...
    <form action="{{ route('admin.extracurricullar.store') }}" method="post" enctype="multipart/form-data">
        @csrf
            <div class="w-full flex flex-col justify-between mt-3 mb-4">
                <div class="w-full mb-5">
                    ...
                    <input name="name" value="{{ old('name') }}" type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan Ekstrakulikuler" required>
                    @error('name')
                ...
                <div class="w-full mb-4">
                    ...
                    <input name="icon"  type="file" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pt-3 pl-[20px] bg-[#FAFAFA]" required>
                    @error('icon')
                        <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                    @enderror
                </div>
