...
<div class="lg:w-[77.5%] lg:h-full md:w-full sm:w-full overflow-hidden">
    ...
    <div class="  min-h-[900px] lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px]  overflow-hidden">
        <p class="font-signika font-medium text-[24px]">Update Ekstrakulikuler</p>

        <div class="form mt-5 mb-5 w-full h-[47px] flex justify-end">
            <a href="{{ route('admin.extracurricullar.index') }}" class="px-3 py-3 rounded-xl text-white font-semibold bg-[#0c720f] block">Kembali</a>
        </div>

    <form action="{{ route('admin.extracurricullar.update', $extracurricullar) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
            <div class="w-full  flex flex-col justify-between mt-3 mb-4">
                <div class="w-full  mb-5">
                    <p class="font-signika font-medium text-[14px] text-[#222831] mb-2">Nama Ekstrakulikuler</p>
                    <input name="name" value="{{ $extracurricullar->name }}" type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan Ekstrakulikuler" required>
                    @error('name')
                ...
                <div class="w-full  mb-4">
                    <p class="font-signika font-medium text-[14px] text-[#222831] mb-2">Foto Ekstrakulikuler</p>
                    <img src="{{Storage::url($extracurricullar->icon)}}" alt="" class="rounded d-block mb-3 h-[120px]" width="120" height="80">
                    <input name="icon"  type="file" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pt-3 pl-[20px] bg-[#FAFAFA]" >
                    <input type="hidden" name="oldImage" value="{{ $extracurricullar->icon }}">
                    @error('icon')
                        <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                    @enderror
                </div>
