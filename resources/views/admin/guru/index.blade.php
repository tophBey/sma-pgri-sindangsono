...
<div class="lg:w-[77.5%] lg:h-full md:w-full sm:w-full overflow-hidden">
            ...
            <!-- header -->
               <div class="lg:mt-2 md:mt-28 sm:mt-28 sm:w-full md:w-full mb-3 lg:w-full h-[85px] flex justify-between mt-4 rounded-xl bg-[#FFFFFF] py-3 px-5 lg:max-w-[1345px] overflow-hidden">
                <p class="font-signika  lg:text-[20px] md:text-[20px] lg:mr-[200px] md:mr-[200px] sm:mr-[200px]  lg:w-full  font-semibold my-auto">Guru</p>
            ...
            <div class=" min-h-screen lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px] overflow-y-scroll overflow-hidden">
                <p class="font-signika font-medium text-[24px]">List Guru</p>

                @if (session()->has('successTeacher'))
                <div id="message" class="w-full h-[70px] bg-[#30a63a] rounded-xl flex px-[20px] mt-3 pt-2">       
                    <div>
                        ...
                        <p class="font-signika font-semibold text-white">{{ session('successTeacher') }}</p>
                    </div>
                </div>
                 @endif

                @if (session()->has('FailedTeacher'))
                    <div id="message" class="w-full h-[70px] bg-[#FFF3E8] rounded-xl flex px-[20px] mt-3 pt-2">       
                        <div>
                            ...
                            <p class="font-signika font-semibold text-white">{{ session('FailedTeacher') }}</p>
                        </div>
                    </div>
                @endif
                <div class="w-full flex mt-8">
                    <div class="lg:w-[55%] md:w-[55%] sm:w-[95%]">
                        <form action="" method="get" class="flex gap-4">
                            <div class="w-[55%] mb-5">
                                <input name="search" value="{{ request('search') }}" type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[44px]  rounded-xl pl-[20px] bg-[#FAFAFA]" autocomplete="off" placeholder="Cari Guru">
                            </div>
                            <div class="">
                ...
                <div class="form mt-5 mb-5 w-full h-[47px] flex justify-end">
                    <a href="{{ route('admin.teacher.create') }}" class="px-3 py-3 rounded-xl text-white font-semibold bg-[#0c720f] block">Tambah Guru</a>
                </div>
                    ...
                    <table  class="w-full">
                        <thead>
                            <tr class="bg-[#30a63a] h-[50px] ">
                                ...
                                <th class="w-[211.6px] font-signika font-semibold text-white text-[14px] text-start p-[10px]">Nama Guru</th>
                                <th class="w-[211px] font-signika font-semibold text-white text-[14px] text-start p-[10px]">Gelar</th>
                                <th class="w-[211px] font-signika font-semibold text-white text-[14px] text-start p-[10px]">Foto</th>
                                <th class="w-[100px] font-signika font-semibold text-white text-[14px] text-start p-[10px] rounded-tr-xl">Aksi</th>
                        ...
                        <tr>
                                ...
                                <td class="border w-[211.6px] p-[10px] font-signika font-light text-[14px]">{{ $teacher->name }}</td>
                                <td class="border w-[211.6px] p-[10px] font-signika font-light text-[14px]">{{ strtoupper($teacher->degrees->name) }}</td>
                                <td class="border w-[211.6px] p-[10px] font-signika font-light text-[14px]"><img class="w-12 h-12 rounded-xl" src="{{ Storage::url($teacher->foto) }}" alt=""></td>
                                <td class="border w-[211.6px] p-[10px] font-signika font-light text-[14px] pl-[20px]">

                                    <div class="flex gap-3">
                                        
                                        <a href="{{ route('admin.teacher.edit', $teacher) }}" class=" flex justify-center items-center w-10 h-10 bg-sky-500 rounded-xl text-[1.2rem]"><i class="fa-solid fa-square-pen"></i></a>

                                        <form action="{{ route('admin.teacher.destroy', $teacher) }}"  method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                        ...
                        @empty
                        <tr>
                            <td colspan="5"><p class="font-bold text-center text-2xl mt-4">Guru Belum tersedia</p></td>
                        </tr>
                        @endforelse
