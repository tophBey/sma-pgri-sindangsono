...
@section('content')
<div class="lg:w-[77.5%] md:w-full lg:h-full overflow-hidden">
  ...
  <!-- header -->
    ...
    <div class="  min-h-[900px] lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px]  overflow-hidden">
        <p class="font-signika font-medium text-[24px]">Update Berita</p>

        <div class="form mt-5 mb-5 w-full h-[47px] flex justify-end">
            <a href="{{ route('admin.news.index') }}" class="px-3 py-3 rounded-xl text-white font-semibold bg-[#0c720f] block">Kembali</a>
        </div>
        ...
        @method('put')
            <div class="w-full  flex flex-col justify-between mt-3 mb-4">
                <div class="w-full  mb-5">
                    ...
                    <input name="title" value="{{ $news->title }}" type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan judul" required>
                    @error('title')
                ...
                <div class="w-full  mb-4">
                    <p class="font-signika font-medium text-[14px] text-[#222831] mb-2">Foto Thumbnail</p>
                    <img src="{{Storage::url($news->thumbnail)}}" alt="" class="rounded d-block mb-3 h-[120px]" width="120" height="80">

                    <input name="thumbnail"  type="file" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pt-3 pl-[20px] bg-[#FAFAFA]" >
                ...
                <div class=" w-full mt-4">
                    ...
                    <textarea  name="description" id="" cols="30" rows="10" class="focus:outline-none min-h-[130px] focus:ring focus:ring-[#0c720f] w-full h-[90px] rounded-xl pl-[20px] bg-[#FAFAFA] pt-[10px]" placeholder="Deskripsi Berita">{{ strip_tags($news->description) }}</textarea>
                    @error('description')
                            <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                    @enderror
                    </div>
