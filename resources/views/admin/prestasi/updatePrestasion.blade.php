...
<div class="lg:w-[77.5%] lg:h-full overflow-hidden">
    ...
    <div class="  min-h-[900px] lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px]  overflow-hidden">
        <p class="font-signika font-medium text-[24px]">Update Prestasi</p>
    ...
    <form action="{{ route('admin.prestasion.update', $prestasion) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
            <div class="w-full  flex flex-col justify-between mt-3 mb-4">
                <div class="w-full  mb-5">
                    ...
                    <input name="title" value="{{ $prestasion->title }}" type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan judul prestasi" required>
                    @error('title')
                ...
                <div class="w-full  mb-4">
                    ...
                    <img src="{{Storage::url($prestasion->photo_prestasion)}}" alt="" class="rounded d-block mb-3 h-[120px]" width="120" height="80">
                    ...
                    <input type="hidden" name="oldImage" value="{{ $prestasion->photo_prestasion }}">
                ...
                <div class="w-full  mb-5">
                    ...
                    <input name="date" value="{{ $prestasion->date }}" type="date" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" required>
                    @error('date')
                        <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div class="w-full h-[80px] mb-5">
                    ...
                    <select name="category_id" id="" class="w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pt-1 pl-[20px] bg-[#FAFAFA]">
                        <option value="">Pilih Kategori</option>
                        @foreach ($categories as $category )
                            @if (old('category_id', $prestasion->category_id) === $category->id)
                ...
                <div class=" w-full mt-4">
                    ...
                    <textarea  name="description" id="" cols="30" rows="10" class="focus:outline-none min-h-[130px] focus:ring focus:ring-[#0c720f] w-full h-[90px] rounded-xl pl-[20px] bg-[#FAFAFA] pt-[10px]" placeholder="Masukan Deskripsi Prestasi">{{ $prestasion->description }}</textarea>
                    @error('description')
                            <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                    @enderror
                    </div>
