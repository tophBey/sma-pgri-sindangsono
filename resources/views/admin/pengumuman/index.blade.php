...
<div class=" min-h-screen lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px] overflow-y-scroll overflow-hidden">
    <p class="font-signika font-medium text-[24px]">List Pengumuman</p>

    @if (session()->has('successAnnountcement'))
        <div id="message" class="w-full h-[70px] bg-[#30a63a] rounded-xl flex px-[20px] mt-3 pt-2">       
            <div>
                ...
                <p class="font-signika font-semibold text-white">{{ session('successAnnountcement') }}</p>
            </div>
        </div>
    @endif

    @if (session()->has('FailedAnnouncement'))
        <div id="message" class="w-full h-[70px] bg-[#FFF3E8] rounded-xl flex px-[20px] mt-3 pt-2">       
            <div>
                ...
                <p class="font-signika font-semibold text-white">{{ session('FailedAnnouncement') }}</p>
            </div>
        </div>
    @endif
    ...
    <div class="w-full flex mt-8">
        <div class="lg:w-[55%] md:w-[55%] sm:w-[95%]">
            <form action="" method="get" class="flex gap-4">
                <div class="w-[55%] mb-5">
                    <input name="search" value="{{ request('search') }}" type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[44px]  rounded-xl pl-[20px] bg-[#FAFAFA]" autocomplete="off" placeholder="Cari Pengumuman">
                </div>
                <div class="">
    ...
    <div class="form mt-5 mb-5 w-full h-[47px] flex justify-end">
        <a href="{{ route('admin.announcement.create') }}" class="px-3 py-3 rounded-xl text-white font-semibold bg-[#0c720f] block">Tambah Pengumuman</a>
    </div>
        ...
        <table  class="w-full">
            <thead>
                <tr class="bg-[#30a63a] h-[50px] ">
                    ...
                    <th class="w-[211.6px] font-signika font-semibold text-[14px] text-start text-white p-[10px]">Deskripsi</th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="border text-center w-[100px] lg:max-h-20 md:min-h-20 md:max-h-20 sm:min-h-[69px] p-[10px] font-signika font-light text-[14px]">{{ $loop->iteration }}</td>
                    <td class="border w-[100px] lg:max-h-20 md:min-h-20 md:max-h-20 sm:min-h-[69px] p-[10px] font-signika font-light text-center text-[14px] pl-[20px]">{{ $announcement->title }}</td>
                    <td class="border lg:max-h-20 md:min-h-20 md:max-h-20 sm:min-h-[69px] p-[10px] font-signika font-light text-[14px] line-clamp-3">{{ $announcement->description }}</td>
                    <td class="border w-[211.6px] lg:max-h-20 md:min-h-20 md:max-h-20 sm:min-h-[69px] p-[10px] font-signika font-light text-[14px] pl-[20px]">

                        <div class="flex gap-3">
                            <a href="{{ route('admin.announcement.edit', $announcement) }}" class=" flex justify-center items-center w-10 h-10 bg-sky-500 rounded-xl text-[1.2rem]"><i class="fa-solid fa-square-pen"></i></a>
                            <form action="{{ route('admin.announcement.destroy', $announcement) }}"  method="post" class="d-inline">
                                @csrf
                                @method('delete')
                ...
                @empty
                <tr>
                     <td colspan="4"><p class="font-bold text-center text-2xl mt-4">Pengumuman Belum tersedia</p></td>
                </tr>
                @endforelse
