...
<div class="lg:w-[77.5%] lg:h-full md:w-full sm:w-full overflow-hidden">
            ...
            <div class=" min-h-screen lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px] overflow-y-scroll overflow-hidden">
                <p class="font-signika font-medium text-[24px]">Tambah Kategori</p>
                ...
                @csrf
                    <div class="w-full  flex flex-col justify-between mt-3">
                        <div class="w-full mb-5">
                            ...
                            <input name="name" value="{{ old('name') }}"  type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan Nama Kategori" required>
                            @error('name')
                                <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                            @enderror
                        </div>

                        <div class="w-full flex h-[80px] justify-end mt-2">
