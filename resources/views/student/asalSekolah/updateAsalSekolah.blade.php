...
<div class=" min-h-[1050px] lg:max-w-[1345px]  w-full  bg-[#FFFFFF] mt-3 rounded-xl p-[20px] overflow-y-scroll overflow-hidden">
    <p class="font-signika font-medium text-[24px]">Update Sekolah Asal</p>
   ...
   <form action="{{ route('student.update-school', $school) }}" method="post">
    @csrf
    @method('put')
        <div class="w-full flex flex-col justify-between mt-3">
            <div class="w-full mb-5">
                <p class="font-signika font-medium  text-[#222831] mb-2">Nama Sekolah</p>
                <input name="shcool_name" value="{{ $school->shcool_name }}" type="text" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan Nama Sekolah" required>
                @error('shcool_name')
            ...
            <div class="w-full mb-5">
                ...
                <input name="graduation_year" value="{{ $school->graduation_year }}" type="number" min="0" class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan nik KTP" required>
            ...
            <div class=" w-full">
                ...
                <div class="w-full ">
                   <textarea  name="address" id="" cols="30" rows="10" class="focus:outline-none min-h-[130px] focus:ring focus:ring-[#0c720f] w-full h-[90px] rounded-xl pl-[20px] bg-[#FAFAFA] border pt-[10px]" placeholder="Alamat Siswa" required>{{ $school->address }}</textarea>
                   @error('address')
                        <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                    @enderror
                </div>
