   ...
   <form action="{{ route('student.update.student', $student) }}" method="post" enctype="multipart/form-data">
    ...
    @method('put')
        <div class="w-full flex flex-col justify-between mt-3">
            ...
            <div class="w-full mb-5">
                <p class="font-signika font-medium  text-[#222831] mb-2">Agama</p>
                <input name="religion" value="{{ $student->religion }}" type="text"  class=" focus:outline-none w-full border  focus:ring focus:ring-[#0c720f]  h-[55px] rounded-xl pl-[20px] bg-[#FAFAFA]" placeholder="Masukan Agama" required>
                @error('religion')
                    <span class="text-red-500 mt-1 font-semibold ml-6 block py-1 text-sm">{{ $message }}</span>
                @enderror
            </div>
