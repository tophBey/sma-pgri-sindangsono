...
<div class="mt-28 flex flex-wrap gap-4  w-full bg-white min-h-96 rounded-xl py-4">
  <div class="w-[90%] mx-auto ">
    <h1 class="text-center text-2xl font-semibold mb-4">{{ $activity->title }}
    </h1>
    ...
    <div class="w-[90%] mx-auto border rounded-lg mb-8 ">
        <img class="w-[100%] max-h-[550px] rounded-lg " src="{{ Storage::url($activity->photo_activity) }}" alt="">
    </div>
    <div class=" px-3 py-4 w-[90%]   mx-auto">
        <p class="text-xl text-justify  mb-14">{{ $activity->description }}</p>
        <div class="flex justify-between mt-3">
            <a href="{{ route('front.activity') }}" class="px-3  py-3 text-xl font-semibold rounded-xl text-[#0c720f] border-2 border-[#0c720f]">Kembali</a>
        </div>
    </div>
  </div>
</div>
